...
class FtpServerController {
    ...
    public async firmarDocumentoDesdeFTP(req: Request, res: Response): Promise<any> {
        ...
        try {
          ...
          // Reconversión para acentos/ñ: node-forge puede devolver strings en Latin1,
          // así que reconstruimos bytes y los decodificamos a UTF-8.
          let nombreFirmante = "Firmante desconocido";
          if (cnAttr?.value) {
            if (typeof cnAttr.value === "string") {
              const raw = cnAttr.value as string;
              // Interpretar los bytes como latin1 y convertirlos a UTF-8
              nombreFirmante = Buffer.from(raw, "latin1").toString("utf8");
            } else if (Array.isArray(cnAttr.value)) {
              // Si viene como array de partes, unimos y aplicamos la misma conversión a cada parte
              nombreFirmante = cnAttr.value
                .map(part => typeof part === "string" ? Buffer.from(part, "latin1").toString("utf8") : String(part))
                .join(" ");
            } else {
              nombreFirmante = String(cnAttr.value);
            }
          }
          ...
          const opcionesFirma = {
            ...
            annotationAppearanceOptions: {
              signatureCoordinates: { left: 0, bottom: 750, right: 400, top: 830 },
              signatureDetails: [
                { value: "Firmado por: " + nombreFirmante, fontSize: 5, transformOptions: { xPos: 20, yPos: 50, space: 1, rotate: 0, tilt: 0 } },
                { value: "Cargo: " + cargo, fontSize: 5, transformOptions: { xPos: 20, yPos: 45, space: 1, rotate: 0, tilt: 0 } },
                { value: "Fecha: " + fechaStr, fontSize: 5, transformOptions: { xPos: 20, yPos: 40, space: 1, rotate: 0, tilt: 0 } },
                { value: "Hora: " + horaStr, fontSize: 5, transformOptions: { xPos: 20, yPos: 35, space: 1, rotate: 0, tilt: 0 } }
              ]
            }
          };
