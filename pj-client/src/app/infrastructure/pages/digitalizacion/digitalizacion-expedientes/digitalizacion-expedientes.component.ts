...
export class DigitalizacionExpedientesComponent implements OnInit {
  ...
  EventAction() {
    if (this.modificarDigitalizacion) {
      this.ModificarDigitalizacion(this.data_expediente_temp.nro_expediente + '.pdf');
  ...
  async ModificarDigitalizacion(nameFile: string): Promise<void> {
    const usuario = this.credencialesService.credenciales;
    const expediente = this.id_expediente_temp;
    const archivo = this.file;
    const isArchivoCargado = !!archivo;

    try {
      // ✅ 1. Calcular hash y detectar cambios
      const hashActual = isArchivoCargado
        ? await getFileHash(archivo!)
        : this.data_digitalizacion.hash_doc;

      const hashPrevio = this.data_digitalizacion.hash_doc;
      const archivoModificado = hashActual !== hashPrevio;

      // ✅ 2. Construir payload de metadatos
      const dataDigitalizacion: DigitalizacionRequest = {
        ...
        escala_gris: this.data_digitalizacion.escala_gris,
        color: this.data_digitalizacion.color,
        observaciones: this.ListObservacionesDigitalizacion.length
          ? this.ListObservacionesDigitalizacion.join('|')
          : null,
        dir_ftp: this.folderPathDocument,
        hash_doc: hashActual,
        peso_doc: isArchivoCargado ? archivo!.size : this.data_digitalizacion.peso_doc,
      ...
      if (erroresValidacion.length > 0) {
        const errorMensaje = erroresValidacion
          .map(e => `Error en el campo "${e.campo}": ${e.mensaje}`)
          .join('\n');
        alert(errorMensaje);
      ...
      // ✅ 4. Función auxiliar para actualizar datos
      const actualizarDatos = () => {
        this.digitalizacionService.ModificarDatosDigitalizacion(expediente, dataDigitalizacion).subscribe({
          ...
          error: (error) => {
            console.error('❌ Error al actualizar datos de digitalización:', error);
            alert('Error al actualizar los datos en el servidor.');
          },
          complete: () => {
            console.log('✔️ Modificación completada');
            this.EstadoDigitalizacionTrabajado();
            this.ModificarRespuestaMensaje();
            this.closeModalDigitalizacion();
            this.sweetAlert.MensajeSimpleSuccess(
              'Expediente Modificado',
              `Expediente ${this.data_preparacion_header.nro_expediente} modificado con éxito.`
            );
          },
        });
      };

      // ✅ 5. Si no hay archivo o no fue modificado → solo actualizar metadatos
      if (!isArchivoCargado) {
        console.warn('⚠️ No se cargó ningún archivo nuevo. Solo se actualizarán los metadatos.');
        actualizarDatos();
        return;
      }

      if (!archivoModificado) {
        console.info('ℹ️ El archivo no ha cambiado. Solo se actualizarán los metadatos.');
        actualizarDatos();
        return;
      }

      // ✅ 6. Procesar y subir archivo si fue modificado
      console.log('📂 Procesando archivo PDF nuevo...');
      ...
      if (originalPdf.getPageCount() === 0) {
        alert('El PDF no contiene páginas.');
        return;
      }

      // Crear portada (primera página)
      const newPdf = await PDFDocument.create();
      const [firstPage] = await newPdf.copyPages(originalPdf, [0]);
      newPdf.addPage(firstPage);

     const portadaFile = new File([Uint8Array.from(await newPdf.save()).buffer], `${nameFile}`, { type: 'application/pdf' });



      // const pdfBytes = await newPdf.save();

      // const portadaFile = new File([pdfBytes.buffer], `${nameFile}.pdf`, {type: 'application/pdf',});

      // ✅ Subida de portada
      console.log('🚀 Subiendo portada...');
      await this.subirArchivoFTP(portadaFile, this.folderPathPortada!, nameFile, 'portada');

      // ✅ Subida del documento completo
      console.log('🚀 Subiendo documento completo...');
      await this.subirArchivoFTP(archivo!, this.folderPathDocument!, nameFile, 'documento completo');

      // ✅ Actualizar datos luego de subir ambos
      actualizarDatos();
    } catch (error) {
      console.error('❌ Error en ModificarDigitalizacion:', error);
      alert('Ocurrió un error al modificar la digitalización. Verifica la consola para más detalles.');
    }
  }

  /**
   * ✅ Método auxiliar para subir archivos al FTP con promesa
   */
    ...
    return new Promise((resolve, reject) => {
      this.sftpService.uploadFile(file, folderPath, nameFile).subscribe({
        ...
        error: (error) => {
          ...
          alert(`Error al subir ${tipo}. Detalle: ${error?.message || 'Desconocido'}`);
          reject(error);
        },
        complete: () => {
          console.log(`✔️ ${tipo} subida completa`);
          resolve();
        },
      });
    });
